...
def encode_spatial_spread_asym(img_bgr):
    ...
    for i in range(num_bits):
        start = i * chunk
        end = N if i == num_bits - 1 else (i + 1) * chunk
        seg = out[start:end]
    ...
    out = np.clip(out, 0, 255).reshape(gray.shape).astype(np.uint8)
...
def process_one_image(path):
    ...
    cv2.imwrite(os.path.join(out_enc, "spatial_spread_geo.png"), geo_img)

    v2, ber2, bits_geo, _ = detect_spatial_spread_asym(geo_img)
    ...
    wb.save(os.path.join(out_cmp, "Comparison_spatial_spread_asym.xlsx"))
