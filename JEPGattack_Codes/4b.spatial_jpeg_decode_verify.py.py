...
JPEG_DIR = os.path.join(BASE_DIR, "Spatial_jpeg")
COMPARISON_DIR = os.path.join(BASE_DIR, "Comparison_spatial_jpeg_result")
...
def main():
    ...
    if not q_dirs:
        ...
        return
    per_image_csv = os.path.join(COMPARISON_DIR, "per_image_spatial_jpeg.csv")
    summary_csv = os.path.join(COMPARISON_DIR, "summary_spatial_jpeg.csv")
    summary_xls = os.path.join(COMPARISON_DIR, "summary_spatial_jpeg.xls")
    per_image_rows = []
    summary_rows = []
    for q, q_path in q_dirs:
        ...
        for folder in folders:
            img_path = os.path.join(q_path, folder, "spatial_spread.jpg")
            ...
            if img is None:
                continue
            total += 1
            verified, ber = detect_spatial_spread_asym(img)
            if verified:
                ok += 1
            ber_sum += float(ber)
